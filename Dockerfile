...
#   docker build -t HASHIRAMA21/yolov8-tracking-module .
...
RUN git clone --recurse-submodules https://github.com/HASHIRAMA21/yolov8-tracking-module.git /usr/src/app
...
#
#       docker run -it --gpus all HASHIRAMA21/yolov8-tracking-module bash
#
#   - run interactively with first and third GPU accessible:
#
#       docker run -it --gpus '"device=0, 2"' HASHIRAMA21/yolov8-tracking-module bash
...
#
#       docker run -it --gpus all -d HASHIRAMA21/yolov8-tracking-module
#
